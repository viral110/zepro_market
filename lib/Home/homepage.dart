...
class _HomePageState extends State<HomePage> {
  ...
  fetchMainSingleBannerHome() async {
    ...
    final response = await ApiServices().mainBannerHome(context);
  ...
  fetchNewArrival() async {
    ...
    final response = await ApiServices().getHomeNewArrival(context);
    var decoded = jsonDecode(response.body);
    if (response.statusCode == 200) {
      ...
      setState(() {
        List.generate(hna.response.length, (index) {
          if (hna.response[index].isTrending == true) {
            storeIndex.add(hna.response[index].productId);
                                                    ...
                                                    child: hna.response[index]
                                                        ...
                                                        ? RichText(
                                                            text: TextSpan(
                                                              children: [
                                                                TextSpan(
                                                                  text: '${hna.response[index].title.capitalizeFirstofEach}'
                                                                      .substring(
                                                                          0,
                                                                          30),
                                                                  style:
  ...
  Widget trending1() {
    return Padding(
      ...
      child: Container(
        child: ListView.builder(
          shrinkWrap: true,
          itemCount: storeImageTrending.length,
          scrollDirection: Axis.horizontal,
          itemBuilder: (context, index) {
            return Row(
              children: [
